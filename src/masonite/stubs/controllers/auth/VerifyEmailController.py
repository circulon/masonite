...
class VerifyEmailController(Controller):
    def verify(
        ...
        application.make("event").fire("auth.email_verified", user)

        return response.redirect("/home").with_success(
            ["Your email has been verified."]
        )

    def notice(self, request: Request, response: Response, view: View, auth: Auth):
        if auth.user() and auth.user().has_verified_email():
            return response.redirect("/home")
        return view.render("auth.verify_email_notice")
